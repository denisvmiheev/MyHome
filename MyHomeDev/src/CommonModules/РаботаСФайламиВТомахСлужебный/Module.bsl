...
Процедура ОчиститьУдаленныеФайлыВТоме(Том)
	РеквизитыТома = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Том, "ВремяПоследнейОчисткиФайлов, ПометкаУдаления, ПолныйПутьLinux, ПолныйПутьWindows");
	...
	Если ОбщегоНазначения.РазделениеВключено() 
			И (ЗначениеЗаполнено(РеквизитыТома.ПолныйПутьLinux) И СтрНайти(РеквизитыТома.ПолныйПутьLinux, "%z") = 0
				ИЛИ ЗначениеЗаполнено(РеквизитыТома.ПолныйПутьWindows) И СтрНайти(РеквизитыТома.ПолныйПутьWindows, "%z") = 0) Тогда
