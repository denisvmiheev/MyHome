...
Процедура УстановитьОформлениеБалансаБюджета()
	...
	Если ИспользоватьПланированиеБюджета = Истина Тогда
		Элементы.ГруппаБалансБюджета.Видимость = Истина;
	Иначе
		Элементы.ГруппаБалансБюджета.Видимость = Ложь;
	КонецЕсли;
...
Процедура РассчитатьБалансБюджета()
	...
	Элементы.НадписьБалансБюджета.Заголовок = СуммаБаланса;
	
	ШаблонЗаголовкаБаланса = НСтр("ru = 'Баланс бюджета (%1)'");
	Элементы.ГруппаКолонка3.Заголовок = СтрШаблон(ШаблонЗаголовкаБаланса, Формат(ДатаПроверки, "ДФ=dd.MM.yyyy;"));
