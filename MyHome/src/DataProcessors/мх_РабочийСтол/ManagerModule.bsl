...
Процедура ПолучитьРасходы(Параметры) Экспорт
	...
	Валюта = мх_КошелькиИСчета.ВалютаПоУмолчанию();
	
	СуммаРасхода		 = СуммаРасходаЗаПериод(Период.ДатаНачала, Период.ДатаОкончания);
	ПредставлениеСуммы	 = мх_РабочийСтол.ПредставлениеСуммы(СуммаРасхода, Валюта, Истина);
...
Функция СуммаРасходаЗаПериод(ДатаНачала, ДатаОкончания)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(РасходыОбороты.СуммаОборот, 0) * ЕСТЬNULL(КурсыВалют.Курс, 1) КАК СуммаОборот
	|ИЗ
	|	РегистрНакопления.мх_Расходы.Обороты(&ДатаНачала, &ДатаОкончания, , ) КАК РасходыОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаОкончания, ) КАК КурсыВалют
...
Процедура ПолучитьБалансБюджета(Параметры) Экспорт    
	...
	ПредставлениеБаланса = мх_РабочийСтол.ПредставлениеСуммы(СуммаБаланса, Валюта, Истина);
	...
	ПредставлениеБаланса = мх_РабочийСтол.ПредставлениеСуммы(СуммаБалансНаличные, Валюта, Истина);
	Результат.Вставить("БалансБюджета_Наличные", ПредставлениеБаланса);
	
	СуммаБалансКредитные = ДолгиПоКредитнымСчетам(ДатаПроверки);
	ПредставлениеБаланса = мх_РабочийСтол.ПредставлениеСуммы(-СуммаБалансКредитные, Валюта, Истина);
	Результат.Вставить("БалансБюджета_Кредитные", ПредставлениеБаланса);
	
	СуммаБалансОбщий = СуммаБалансНаличные - СуммаБалансКредитные;
	ПредставлениеБаланса = мх_РабочийСтол.ПредставлениеСуммы(СуммаБалансОбщий, Валюта, Истина);
...
Функция ОстатокНаличныхДенежныхСредств(Знач Период = Неопределено) Экспорт
	...
	Остаток = Остатки.Итог("СуммаОстатокВРублях");
...
Функция ДолгиПоКредитнымСчетам(Знач Период = Неопределено) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(ЕСТЬNULL(КошелькиИСчета.КредитныйЛимит, 0) 
	|	- ЕСТЬNULL(ДвиженияДенежныхСредствОстатки.СуммаОстаток, 0)), 0) КАК Долг
	|ИЗ
	|	Справочник.мх_КошелькиИСчета КАК КошелькиИСчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.мх_ДвиженияДенежныхСредств.Остатки(&Период, Кошелек В (&Кошельки)) КАК
