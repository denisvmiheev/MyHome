...
Процедура ИнициализироватьТаблицыДляДвижений(ДокументОбъект) Экспорт
	...
	|	ДвиженияДенежныхСредствОстатки.Кошелек.Валюта КАК Валюта,
	|	-ДвиженияДенежныхСредствОстатки.СуммаОстаток КАК Сумма
	|ПОМЕСТИТЬ ПредварительныеДанные
	|ИЗ
	|	РегистрНакопления.мх_ДвиженияДенежныхСредств.Остатки(&Граница, Кошелек = &Кошелек) КАК ДвиженияДенежныхСредствОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВводОстатков.ОбъектУчета,
	|	ВводОстатков.ОбъектУчета.Валюта,
	|	ВводОстатков.Сумма
	|ИЗ
	|	Документ.мх_ВводОстатков КАК ВводОстатков
	|ГДЕ
