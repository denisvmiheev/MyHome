...
Функция ПроверитьЗаполнениеПараметров(Знач Параметры, Знач НаборКлючей) Экспорт
		
	Результат = Новый Структура("Успех, ОписаниеОшибки", Истина, "");
	
	Если ТипЗнч(НаборКлючей) = Тип("Массив") Тогда
		Ключи = НаборКлючей;
	ИначеЕсли ТипЗнч(НаборКлючей) = Тип("Строка") Тогда
		Ключи = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(НаборКлючей, ",", Истина, Истина);
	Иначе
		ВызватьИсключение "Необрабатываемый тип параметра ""НаборКлючей"" функции ""ПроверитьЗаполнениеПараметров""";
	КонецЕсли;
	
	ШаблонОшибки = "В параметрах отсутствует обязательный параметр ""%1""";
	
	МассивОшибок = Новый Массив;
	
	Для каждого ИмяКлюча из Ключи Цикл        
		
		СлучайноеЗначение = Строка(Новый УникальныйИдентификатор);
		
		ЗначениеКлюча = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, ИмяКлюча, СлучайноеЗначение);
		
		Если ЗначениеКлюча = СлучайноеЗначение Тогда
			ТекстОшибки = СтрШаблон(ШаблонОшибки, ИмяКлюча);
...
Функция РезультатСОшибкой(ТекстОшибки, Результат = Неопределено) Экспорт
	
	Если Результат = Неопределено Тогда
		Результат = Новый Структура("Успех, ОписаниеОшибки", Ложь, ТекстОшибки);
	КонецЕсли;
