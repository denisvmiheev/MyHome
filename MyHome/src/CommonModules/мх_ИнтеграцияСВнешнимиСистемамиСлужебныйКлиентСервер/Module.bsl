...
Функция ОтправитьЗапрос(
	...
	Возврат ОтправитьЗапросРекурсивно(
		ПараметрыОтправки, ПолучитьТелоКакСтроку, Таймаут, Перенаправления, Метод)
...
Функция ОтправитьЗапросРекурсивно(
	...
	Если ОтветHTTP.КодСостояния = 301 // 301 Moved Permanently
		...
		Иначе
			...
			//	НовыйURL = СтрШаблон(
			//		"%1://%2:%3/%4", Протокол, СтруктураURI.ИмяСервера, Формат(СтруктураURI.Порт, "ЧГ="), НовыйURL);
			//КонецЕсли;

			//ПараметрыОтправки.АдресСервера = НовыйURL;

			Возврат ОтправитьЗапросРекурсивно(
				ПараметрыОтправки, ПолучитьТелоКакСтроку, Таймаут, Перенаправления);
