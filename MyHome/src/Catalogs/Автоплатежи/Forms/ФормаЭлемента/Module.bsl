...
Процедура ОчиститьЗначениеНеиспользуемойАналитики()
	...
	Если ТипЗнч(Объект.СтатьяДоходовРасходов) = Тип("ПланВидовХарактеристикСсылка.СтатьиДоходов") Тогда
		Объект.АналитикаРасходов = Неопределено;
	ИначеЕсли ТипЗнч(Объект.СтатьяДоходовРасходов) = Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов") Тогда
...
Процедура УстановитьОграничениеТипаДляАналитики()
	...
	Если Объект.СтатьяДоходовРасходов <> Неопределено Тогда
		СобытияФормВызовСервера.УстановитьОграничениеТипаДляАналитикиДоходовРасходов(ЭлементФормыАналитика, Объект.СтатьяДоходовРасходов);
	КонецЕсли;
